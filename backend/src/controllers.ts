...
import { AppService } from './app.service'; import { JwtAuthGuard, LoginThrottleGuard, Public, Roles, RolesGuard, TwilioWebhookGuard } from './auth'; import { CampaignDto, ContactDto, ContactStatusDto, CreateUserDto, ImportContactsDto, LoginDto, MfaEnforcementDto, MfaTokenDto, MfaVerifyDto, SegmentDto, SettingsDto, StatusDto, UpdateUserDto } from './dto';
@ApiTags('Conecta Masivo') @Controller() export class AppController {
 ...
 @Get('contacts') @UseGuards(JwtAuthGuard,RolesGuard) @Roles(UserRole.ADMIN,UserRole.OPERATOR) contacts(@Query() q:Record<string,string>){return this.service.contacts(q)}
 @Get('contacts/export') @UseGuards(JwtAuthGuard,RolesGuard) @Roles(UserRole.ADMIN,UserRole.OPERATOR) async exportContacts(@Res() res:Response){const r=await this.service.contacts({limit:'10000'}); const values=r.data.map(c=>[c.firstName,c.lastName,c.phone,c.email||'',c.departmentOrZone||'',c.groupName||'',c.whatsappOptIn?'SI':'NO',c.smsOptIn?'SI':'NO'].map(v=>'"'+String(v).replace(/"/g,'""')+'"').join(',')).join('\n'); res.header('Content-Type','text/csv').attachment('contactos.csv').send('nombres,apellidos,telefono,email,zona,grupo,consentimiento_whatsapp,consentimiento_sms\n'+values);}
 ...
 @Get('segments') @UseGuards(JwtAuthGuard,RolesGuard) @Roles(UserRole.ADMIN,UserRole.OPERATOR) segments(){return this.service.segments()}
