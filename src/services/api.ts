import { Campaign, Contact, MessageLog, Segment, User } from '../types';
...
export function toUser(value: any): User { return { id:value.id, nombre:value.name, email:value.email, rol:role(value.role), estado:value.isActive?'activo':'inactivo', activo:value.isActive, ultimoAcceso:value.lastLoginAt || undefined, creadoEn:value.createdAt }; }
